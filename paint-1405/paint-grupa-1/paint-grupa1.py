...
def end_draw(event):
    ...
    if(tool == "line"):
        ...
        canvas.delete("preview")
        text = simpledialog.askstring("Tekst", "Podaj tekst:")
        if text:
            canvas.create_text(event.x, event.y, text=text, fill=current_color, font=("Arial", size))
...
text_button = tk.Button(root, text="Text", command=lambda: set_mode("text"))
